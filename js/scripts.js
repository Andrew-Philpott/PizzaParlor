...
function createPizzaSizesHtml(pizzaSizes) {
  ...
  let pizzaSizesHtml = "";
  pizzaSizesHtml +=
    "<div id='pizza-size-select'><h2>Select Pizza size</h2><ul>";
...
function createCustomerPizzaStatusView() {
  ...
  let htmlForCustomerPizzaStatus =
    "<div id='pizza-status-header'><h2>Your Pizza</h2></div><div id='pizza-status'</div>";
...
function updateCustomerPizzaStatusViewWithPizzaSize(size, pizzaCrustType) {
  ...
  htmlForCustomerPizzaStatusUpdate += `<div class='pizza-option-status'><p><span id='selected-pizza-crust-size'>${size}</span><span> > </span><span id='selected-pizza-crust-type'>${pizzaCrustType}</span></p></div>`;
...
function createPizzaOrderHtml(pizzas) {
  ...
  pizzas.forEach(function(pizza) {
    pizzasHtml += `<div class='order-container'><span>${pizza
      .getPizzaCrust()
      .getSize()} ${pizza
      .getPizzaCrust()
      .getType()}</span><span>${pizza.getPizzaCrust().getPrice()}</span></div>`;
    pizza.toppings.forEach(function(topping) {
      pizzasHtml += `<div class='order-container'><span>${topping.getToppingName()}</span><span>${topping.getToppingPrice()}</span></div>`;
    });
  });
  let pizzaTotal = new Number();
  pizzas.forEach(function(pizza) {
    pizzaTotal += parseFloat(pizza.calculatePizzaCost()).toFixed(2);
  });
  pizzasHtml += `<div class='order-container'><hr/><p><span>Total</span><span>$${parseFloat(
    pizzaTotal
  ).toFixed(
    2
  )}</span></p></div><button id='add-pizza-button' class='button-theme'>Add Pizza</button></div>`;
  ...
  $("#review-pizza-order-button").on("click", function() {
    ...
    for (let i = 0; i < toppingsSelected.length; i++) {
      ...
      let newTopping = new Topping(
        toppingNameAndPriceArray[0],
        toppingNameAndPriceArray[1]
      );
